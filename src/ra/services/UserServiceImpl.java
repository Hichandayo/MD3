...
public class UserServiceImpl implements IGenericDesign<User> {
    ...
    public boolean updatePassword(String email, String newPassword) throws IOException, ClassNotFoundException {
        List<User> users = readFromFile(CUSTOMER_PATH);
        for (User user : users) {
            if (user.getEmail().equals(email)) {
                user.setPassword(newPassword); // Update the password
                writeToFile(CUSTOMER_PATH, users); // Save the updated user list
                return true;
            }
        }
        return false;
    ...
    private void loadUsersFromFile() {
        ...
        try {
            List<User> loadedUsers = IOFile.readFromFile(CUSTOMER_PATH);
    ...
    private void saveUsersToFile() {
        ...
        try {
            writeToFile(CUSTOMER_PATH, getAll());
